...
public class SyncSingleClient extends ClientConnectionHandler {
	...
	public void finalize() {
        CmmUtil.print("i", "I'm SingleClientMgr...bye!!");
	}
	...
	public void OnReceivePacket(byte[] buf, int size) {
		...
		try {
			...
			do {
				...
				if (uploadingFile != null) {
					if (uploadFile_remainSize <= bufRemain) {
						...
						CmmUtil.print("i", "SEND : " + new String(sndBytes));
						
						// release uploading file
						uploadFile_remainSize = 0;
						uploadingFile.Close();
						CmmUtil.print("i", "filePath["+filePath+"] fileName["+fileName+"] Receive complete!!");
					...
					CmmUtil.print("i", "RECV : " + new String(rcvBuf) + " size["+size+"]");
			...
			CmmUtil.print("w", "Incomplete packet. offset :" + offset + ", size : " + size);
		} catch (Exception e) {
			CmmUtil.print("i", e.getMessage());
			offset = size;
		}
		if (size > offset) {
...
//			System.out.println(randomGenerator.nextInt(99999));
			...
			CmmUtil.print("i", "SEND : " + retPack.GetString());
			break;
		case "930" :
			...
			break;
		default:
			CmmUtil.print("w", "Invalid Packet type : " + msgNo);
			break;
		}
...
//		}
	...
	private void SendFilesToClient(String filePath, String fileName,
		...
		if (files == null) {
			CmmUtil.print("i", filePath + " does not exist");
		} else {
			for (int i = 0; i < files.length; i++) {
				if (files[i].isFile()) {
					fileCount += 1;
					fileTotalSize += files[i].length();
					CmmUtil.print("i", "file name : " + files[i].getName() + ", size : " + files[i].length());
				}
			}
		}
	...
	public void SendAFileToClient(File srcFile, File backupFile) throws IOException {
		CmmUtil.print("i", "Transfer Start. file name : " + srcFile.getName() + ", size : " + srcFile.length());
		...
		CmmUtil.print("i", "Transfer Ended. file name : " + srcFile.getName() + ", size : " + srcFile.length());
	}
